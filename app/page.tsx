...
function Status() {
  return(
    <div className="md:flex-3/4 gap-3 flex-row">
    Статус сервера:
    {
      fetchServerActivity().then(data => data.online ? 
        <>
          <div className="text-green-500 bg-neutral-700 rounded-xl p-2 px-4">• онлайн</div>
          Игроков онлайн:
          <div className="bg-neutral-700 rounded-xl p-2 px-4">{data.players.online} / {data.players.max}</div>
        </> : 
...
function DownloadPack() {
  return(
    <div className="md:flex-2 flex-wrap p-5 px-7">
    ...
    <div className="flex md:flex-3 min-w-1/4 not-md:w-full justify-center">Скачать сборку</div>
    {modpacks.map((lnk, i) => 
...
export default function Home() {
  return (
    <div className="flex flex-col h-screen w-screen p-0 m-0 text-xl">
      ...
      *:flex *:bg-neutral-800 *:rounded-2xl *:p-5 *:px-7 *:gap-3 *:items-center *:justify-center *:shadow-xl
      ">
        <Link href={""} className="w-3/5 h-60 bg-[url('/Ad_Banner.png')] bg-cover bg-center">
          <p className="text-2xl font-black text-shadow-lg text-shadow-gray-950 bg-opacity-20">Добро пожаловать в первый сезон NeeuCraft</p>
        </Link>   

        <Link href={"discord://invite/meeubase"} className="flex-1 h-60 bg-[url('https://cdn.discordapp.com/icons/1161185029154676736/f707ae98a5c47717560d9056d44df6c8.webp?size=1024')] bg-cover bg-center">
          <p className="text-2xl font-black text-shadow-lg text-shadow-gray-800">Discord сервер Meeu Base</p>
        </Link>
